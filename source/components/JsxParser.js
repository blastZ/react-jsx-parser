...
export default class JsxParser extends Component {
  ...
  parseJSX = rawJSX => {
    ...
    const result = parsed.map(this.parseExpression);
  ...
  parseExpression = expression => {
    switch (expression.type) {
      ...
      case 'JSXText':
        ...
        return this.props.disableFragments ? expression.value : <Fragment key={key}>{expression.value}</Fragment>;
      case 'JSXAttribute':
      ...
      case 'MemberExpression':
        if (expression.object.type === 'Identifier' && expression.property.type === 'Literal') {
          return (this.props.bindings || {})[expression.object.name][expression.property.value];
        }

        if (expression.object.type === 'Identifier' && expression.property.type === 'Identifier') {
          return (this.props.bindings || {})[expression.object.name][expression.property.name];
        }

        if (expression.object.type === 'MemberExpression' && expression.property.type === 'Literal') {
          return (this.parseExpression(expression.object) || {})[expression.property.value];
        }
      ...
      case 'CallExpression':
        ...
        if (parsedCallee === undefined) {
          this.props.onError(
            new Error(`The expression '${expression.callee}' could not be resolved, resulting in an undefined return value.`)
          );
          return undefined;
        }
  ...
  parseElement = element => {
    ...
    if (children || children === 0) props.children = children;
  ...
  render = () => {
    ...
    return this.props.renderInWrapper ? <div className="jsx-parser">{this.ParsedChildren}</div> : <>{this.ParsedChildren}</>;
  };
}
...
if (process.env.NODE_ENV !== 'production') {
  ...
  JsxParser.propTypes = {
    ...
    blacklistedAttrs: PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.string, PropTypes.instanceOf(RegExp)])),
